...
% 1. All demand, tariff, boiler, turbine, and fuel data now trace back to
%    refinery_cogen_parameters.xlsx (Parameters + Hourly_Profile sheets) instead
%    of independently-invented numbers -- see comments above each block for the
%    grounded/derived/representative basis of every value.
% 2. Removed dead code: unused `nEq` variable (was also miscounted --
%    7 equality rows/hour, not 8) and the unused `addEq` anonymous function.
% 3. End-of-day battery SOC is now an exact equality (= start-of-day SOC),
%    not just a lower bound -- matches the "return to start-of-day SOC" design
%    decision from the parameter sheet.
% 4. FG_MAX_FRACTION (0.6) remains a REPRESENTATIVE, not grounded, combustion-
%    stability assumption -- flagged here since it wasn't sourced from search,
